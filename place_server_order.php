...
/**
 * Script de envio de pedido de Server Service Dhru Fusion em modo de produção.
...
$account = trim((string) filter_input(INPUT_POST, 'account', FILTER_SANITIZE_STRING));
...
function flatten_service_items(array $item)
{
    ...
    if (isset($item['SERVICEID']) || isset($item['serviceid']) || isset($item['ID']) || isset($item['id'])) {
        return [$item];
    }
...
function find_server_service_price(DhruFusion $api, string $serviceId): ?float
{
    $response = $api->action('serverloginservicelist');
...
if ($account === '') {
    echo json_encode(['error' => 'O campo Nome de Usuário / E-mail da Conta é obrigatório para pedidos de Server Services.']);
    exit;
}
...
$servicePrice = find_server_service_price($api, $serviceId);

if ($servicePrice === null) {
    echo json_encode(['error' => 'Não foi possível obter o preço do serviço Server Service.']);
    exit;
}
...
$response = $api->action('placeserverorder', $params);
if ($response === false) {
