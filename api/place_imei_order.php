...
/**
 * Script de envio de pedido de IMEI Dhru Fusion em modo de produção.
...
$imei = trim((string) filter_input(INPUT_POST, 'imei', FILTER_SANITIZE_STRING));
...
function find_imei_service_price(DhruFusion $api, string $serviceId): ?float
{
...
if ($imei === '' || $serviceId === '') {
    echo json_encode(['error' => 'IMEI e ID do serviço são obrigatórios.']);
    exit;
}

if (!preg_match('/^\d{15}$/', $imei)) {
    echo json_encode(['error' => 'IMEI inválido. Deve conter 15 dígitos.']);
    exit;
}
...
$servicePrice = find_imei_service_price($api, $serviceId);

if ($servicePrice === null) {
    echo json_encode(['error' => 'Não foi possível obter o preço do serviço IMEI.']);
    exit;
}
